...
from .models import VideoFaceRecognition, VideoScene, VideoSceneShare, VideoFaceRecognitionShare

sys.path.append("/var/www/html/swpp2021-team7/ml")
import detectScene
import youtube
...
def getScnCut(request):
    ...
    if video_type not in [TYPE_YOUTUBE, TYPE_FILE]:
        return JsonResponse(status=400, data={'status':'false','message':"type error"})
    ...
    if video_type is TYPE_YOUTUBE:
        ...
        return JsonResponse(status=404, data={'status':'false','message':"type error"})
